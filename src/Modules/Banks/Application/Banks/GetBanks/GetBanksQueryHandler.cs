...
internal class GetBanksQueryHandler : IQueryHandler<GetBanksQuery, List<BankDto>>
{
    ...
    public async Task<List<BankDto>> Handle(GetBanksQuery query, CancellationToken cancellationToken)
    {
        ...
        var sql = @"SELECT id, external_id AS externalId, name, country_code AS countryCode, external_provider_name AS externalProviderName, status, 
                    last_banks_synchronisation_process_id AS lastBanksSynchronisationProcessId, max_consent_days AS maxConsentDays, 
                    is_regulated AS isRegulated, default_logo_url AS defaultLogoUrl, logo_url AS logoUrl, created_at AS createdAt, updated_at AS updatedAt FROM banks.banks";

        var banks = await connection.QueryAsync<BankDto>(sql);
