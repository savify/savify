...
[TestFixture]
public class GetBankInternalQueryTests : TestBase
{
    [Test]
    public async Task GetBankQuery_WhenBankExists_ReturnsBank()
    {
        ...
        Assert.That(bank.ExternalProviderName, Is.EqualTo(ExternalProviderName.SaltEdge.Value));
        Assert.That(bank.Status, Is.EqualTo(BankStatus.Beta.Value));
        Assert.That(bank.MaxConsentDays, Is.EqualTo(90));
        Assert.That(bank.IsRegulated, Is.True);
